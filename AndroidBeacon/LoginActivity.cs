	...
	public class LoginActivity : Activity 
	{
		...
		protected override void OnCreate (Bundle bundle)
		{
			...
			btnSignIn.Click += async (sender, e) => {
				...
				while (i< nbAccounts && ! trouve) {
					...
					string url = "http://192.168.1.14:8383/IBeaconService/rest/ClientServiceWS/authenticateUser?login="+
		...
		public void saveArray()
		{
			...
			for(int i=0;i<services.Count();i++)  
			{
				...
				loginEditor.Remove("BeaconService_" + i);
				loginEditor.PutString("BeaconService_" + i,valeurs[2]);  
		...
		private void ParseAndProcess (JsonValue json)
		{
			...
			if (json["idUser"] == null) {
				...
				foreach (PrivateService pService in userInfo.privateServices) {
					services.Add (userInfo.idUser+"|"+pService.serviceName+" "+pService.serviceDescription+"|B9407F30F5F8466EAFF925556B57FE6D");
				}
